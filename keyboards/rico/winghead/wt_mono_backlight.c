...
#include "wt_backlight_api.h" // reuse these for now
#include "wt_backlight_keycodes.h" // reuse these for now
...
{
	IS31FL3733_set_value_all( value );
}

void backlight_effect_all_off(void)
{
	IS31FL3733_set_value_all( 0 );
}

void backlight_effect_all_on(void)
{
	IS31FL3733_set_value_all( g_config.brightness );
}
...
void backlight_effect_raindrops(bool initialize)
{
    ...
    for ( int i=0; i<96; i++ )
    {
        ...
        {
            IS31FL3733_set_value(i, rand() & 0xFF );
        }
    }
}
...
void backlight_config_set_value( uint8_t *data )
{
    ...
    switch ( *value_id )
    {
        ...
        case id_color_1:
        {
            // Does nothing as the Windhead does not handle color settings
            break;
        }
    }
}
...
{
	IS31FL3733_update_pwm_buffers(LED_DRIVER_ADDR_1,0x00);
}
